...
def login_lagi334():
	try:
		...
		print(f'  %s[%sx%s]%s LOGIN GAGAL.....COKIES MODARRRR !!%s'%(x,k,x,m,x))
		exit()
def bot():
	try:
...
def dump_massal():
	...
	for userr in uid:
		try:
			col = ses.get('https://graph.facebook.com/v14.0/'+userr+'?fields=friends.limit(5000)&access_token='+tokenku[0], cookies = {'cookies':cok}).json()
